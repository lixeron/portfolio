  ...
  general: [
    {
      ...
      desc: "Open-core adversary simulation platform. It runs realistic phishing, smishing, QR, and credential-harvest campaigns to train staff, then tracks who took the bait.",
      longDesc: "Project Specter is a self-hostable security awareness platform that simulates four social engineering vectors: phishing email, smishing, QR/quishing, and credential-harvesting pages. It generates adaptive attack content, tracks engagement, and pushes training to whoever falls for it. The backend is FastAPI with SQLAlchemy, the dashboard is React, and the whole thing is containerized with a full test suite and CI/CD that publishes images to the GitHub Container Registry, so it stands up with one docker compose up. I built it open-core because I wanted to prove I could architect a real full-stack security product end to end, not just a class demo, and security is the field I am heading into.",
      tech: ["Python", "FastAPI", "React", "Docker", "SQLAlchemy"],
    ...
    },
    {
      ...
      desc: "Programmed a DJI Tello drone to detect and track vehicles in real time with YOLOv8 and OpenCV, feeding a live diagnostic dashboard.",
      longDesc: "This one flies a DJI Tello quadcopter through Python and the official Tello SDK, pulls the live video feed, and runs frame-by-frame object detection with YOLOv8 and OpenCV to classify and track vehicles. A custom Tkinter dashboard shows airspeed, target coordinates, system warnings, and classification graphs. It was a team project for CS 420, and I took the computer vision and flight-control side. I wanted hands-on experience wiring real hardware to a live ML pipeline instead of just running models against a static dataset.",
    ...
    },
    {
      ...
      desc: "A live map of every railroad crossing in Birmingham, backed by real FRA blockage data. Which crossings block, and how often.",
      longDesc: "Trackly maps every railroad grade crossing in Birmingham and layers on historical blockage data from the Federal Railroad Administration, pulled through the free Socrata API. The backend is FastAPI over a PostGIS spatial database using SQLAlchemy and GeoAlchemy2, with ingestion scripts for both the FRA crossing inventory and the blocked-crossing history. The frontend is a React and Leaflet map. Phase 1, static crossings plus historical data, is working, and real-time plus crowdsourced reporting are next. I built it because blocked crossings are a real local problem nobody has good visibility into. They snarl traffic and delay emergency response, and the public data to show it already exists, just not anywhere people can actually see it.",
      tech: ["FastAPI", "React", "Leaflet", "PostGIS", "Docker", "Socrata API"],
      role: "Core Creator",
      year: "2025",
    ...
    },
    {
      ...
      desc: "Paste a YouTube link and get platform-ready posts back in seconds: X threads, LinkedIn posts, IG captions, and newsletters.",
      longDesc: "Prism is an AI content repurposing engine for creators. The FastAPI backend grabs a video transcript with yt-dlp and runs it through a provider-abstracted Gemini 2.5 Flash pipeline to generate platform-specific posts, with tone presets and an optional brand-voice input. The React, Vite, and Tailwind frontend is on Vercel, the backend runs on Railway, and the whole stack is containerized with Docker. I built it as a real tool for creators buried in repurposing busywork, and honestly as a test of whether I could ship a full freemium SaaS end to end, trial, usage limits, and all.",
      tech: ["Gemini 2.5 Flash", "FastAPI", "React", "yt-dlp", "Docker"],
    ...
    },
    {
      ...
      desc: "Voice-first Android cooking app. Say what is in your kitchen and it finds what you can actually cook, all hands-free.",
      longDesc: "Vealthy is a native Android app in Kotlin and Jetpack Compose, built around how people really cook, hands busy and phone across the counter. You speak your ingredients, an on-device SpeechRecognizer parses them offline, and it matches them to recipes and reads the steps back with on-device text-to-speech so you never have to tap the screen mid-cook. Your pantry saves locally between sessions. I built it because I cook from scratch a lot and every recipe app assumes clean hands and endless scrolling. I wanted a cook-what-you-already-have companion that just listens.",
      tech: ["Kotlin", "Jetpack Compose", "SpeechRecognizer", "On-device TTS", "Room", "TheMealDB"],
      role: "Solo Android Creator",
      year: "2025-2026",
      link: "https://github.com/lixeron/Vealthy",
      video: null,
      color: "var(--brass-color)",
      status: "wip"
    }
  ...
  games: [
    {
      id: "dead-route",
      title: "Dead Route",
      tagline: "Terminal Zombie Survival Roguelite",
      desc: "A terminal zombie roguelite where every choice scars you, literally. 19 interconnected systems, 13 NPCs, and a real CI/CD pipeline behind a text UI.",
      longDesc: "Dead Route is a Python survival roguelite that runs in the terminal. You manage scarcity, take risks that leave permanent consequences, and try to outlast an infected world. Under the hood it is a three-layer UI, Engine, and Data architecture with 19 interconnected systems, 13 unique NPCs, a 34-test suite, Docker containerization, and a GitHub Actions CI/CD pipeline, and it is deliberately built to port over to the Godot engine later. The game was the fun part, but the real point was the engineering. I wanted to take something I actually enjoy, games, and wrap it in the same testing and CI discipline I would bring to production software.",
    ...
    },
    {
      ...
      desc: "A dynastic life sim where a life is a run and death is succession. The dynasty is the save file.",
      longDesc: "Aevum is a narrative roguelite life simulator in React Native and Expo, written in TypeScript. Each run takes a character from birth to death through choice-driven events with real risk rolls, so reckless decisions can land you in the ER, in legal trouble, or dead, and hitting zero health ends the line. Choices shuffle every event to stop save-scumming, and three full themes retint every screen. It is at v0.3 right now with the save and succession system and trophies still in progress. I built it to dig into game feel and systems design on the mobile stack I actually like working in, Expo, the same way a lot of indie apps get made.",
      tech: ["React Native", "Expo", "TypeScript", "Expo Haptics", "Expo Audio"],
  ...
  it: [
    {
      ...
      desc: "A beginner-friendly networking CTF that hides a secret flag inside ICMP packets buried in noise traffic. Filter, extract, decode.",
      longDesc: "This is a Capture The Flag challenge I built to teach packet analysis. A Python generator breaks a secret flag into hex chunks, hides them inside ICMP echo requests, and floods a PCAP with HTTP, DNS, and junk traffic to bury it. To solve it, players write tshark filters, pull the payload stream, reverse the hex order, and peel back Base64 layers. It was actually my first ever real project, back in high school, and it is where I got hooked on networking and security in the first place.",
  ...
  shopify: [
    {
      ...
      tagline: "Shopify Product Options Engine",
      desc: "A Shopify app that lets merchants build advanced, conditional product options beyond Shopify's native limits. Free tier plus paid Pro.",
      longDesc: "Anvil is a live Shopify app, approved and on the App Store, that gives merchants advanced product option and customization controls past what Shopify supports natively, including conditional logic and a free plus Pro billing model. It is one of the products under my solo studio, Astoylo. The whole point was to ship something real into a marketplace with actual distribution and a working paid tier, not just another side project sitting in a repo.",
      tech: ["TypeScript", "React", "Shopify API"],
      role: "Lead Architect // Solo Creator",
      year: "2025-2026",
      link: "https://github.com/lixeron",
      liveLink: "https://anvilextension.netlify.app/",
      appLink: "https://apps.shopify.com/anvil-1",
